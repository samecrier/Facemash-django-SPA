...
from frontend.data_helpers import GetData
...
class HomeView(View):
	...
	def get(self, request):
		...
		if request.user.is_authenticated:
			...
			if mode == 'refresh':
				competitor_1, competitor_2 = self.helper_service.get_two_competitors()
				updated_matchup = self.helper_service.update_saved_matchup(
					request.user,
					competitor_1.id,
					'1',
					0,
	...
	def post(self, request):
		if request.headers.get('x-requested-with') == 'XMLHttpRequest':
			...
			new_enemy = self.data_service.get_data_competitor_js(
				winner_id,
				loser_id,
				winner_position
			)
			...
			for competitor in new_enemy:
				if int(winner_id) != new_enemy[competitor]["competitor"]['id']:
					...
					if request.user.is_authenticated:
						self.helper_service.update_saved_matchup(
			...
			winner_rating = self.helper_service.get_winner_rating(winner_id)
